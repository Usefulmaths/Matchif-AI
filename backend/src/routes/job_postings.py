...
def add_user_profile_and_get_vector(user_service: UserService, pdf_text: str) -> List[float]:
...
def get_job_postings(job_service: JobService, user_profile_vector: List[float], limit: int = 10) -> List[Dict[str, Any]]:
    """Retrieve job postings from Weaviate that match the user profile vector."""
    job_postings = job_service.get_job_postings_near_vector(user_profile_vector, limit=limit)
    return [
        {
...
async def upload_and_create_profile(
    ...
    try:
        ...
        user_profile_vector = add_user_profile_and_get_vector(services['user_service'], text)

        # Retrieve job postings
        job_postings_data = get_job_postings(services['job_service'], user_profile_vector)
